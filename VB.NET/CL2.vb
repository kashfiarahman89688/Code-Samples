...
Public Class CL2
    ...
    Public Sub CL2_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        'TODO: This line of code loads data into the 'DataSet3.VW_CL2' table. You can move, or remove it, as needed.
        ...
        Me.VW_CL2TableAdapter.SelectCommand.CommandText = GetParam("fillcl2")
        Me.VW_CL2TableAdapter.Fill(DataSet3.VW_CL2, BranchCode, "%_____________", "%")
        ...
        If flag = True Then
            ...
            'rssql.Open(Sql, conn, ADODB.CursorTypeEnum.adOpenKeyset)
            'Do Until rssql.EOF
            ' Cmbqj1.Items.Add(rssql.Fields("SHORT").Value.ToString)
            'rssql.MoveNext()
            'Loop
        End If
        flag = False
    ...
    Private Sub DataGridViewCL3_DoubleClick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles DataGridViewCL3.DoubleClick
        Try
            ...
            If MsgBoxResult.Yes = MsgBoxResult.Yes Or rssql.Fields("MARK").Value.ToString.Trim = "N" Then
                ...
                txtArrear.Text = Trim(DataGridViewCL3.SelectedRows(0).Cells(22).Value.ToString) + " month/s " + Trim(DataGridViewCL3.SelectedRows(0).Cells(14).Value.ToString) + " day/s"
                ...
                If grp = "0501" Then
                    ...
                    TextBoxdayExp.Enabled = True
                    TextBoxmonExp.Enabled = True
                    TextBoxYearExp.Enabled = True
                    If DataGridViewCL3.SelectedRows(0).Cells(5).Value.ToString <> "" Then
                        TextBoxdayExp.Text = DataGridViewCL3.SelectedRows(0).Cells(5).Value.ToString.Substring(6, 2)
                        TextBoxmonExp.Text = DataGridViewCL3.SelectedRows(0).Cells(5).Value.ToString.Substring(4, 2)
                        TextBoxYearExp.Text = DataGridViewCL3.SelectedRows(0).Cells(5).Value.ToString.Substring(0, 4)
                ...
                Else
                    ...
                    TextBoxYearExp.Text = ""
                    Label11.Visible = True
                    TextBoxdayExp.Enabled = False
                    TextBoxmonExp.Enabled = False
                    TextBoxYearExp.Enabled = False
                ...
                If rs.Fields("OB_STA").Value.ToString <> "" Then

                    Sql = GetParam("oc_stacl2")
                    If rssql.State = 1 Then
                        rssql.Close()
                    End If

                    rssql.Open(Sql + "" & acct_no & "))", conn, ADODB.CursorTypeEnum.adOpenKeyset)
    ...
    Private Sub ButtonCalculate_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ButtonCalculate.Click
        Try
            ...
            dt = TextBoxYearExp.Text.ToString.PadLeft(4, "0") + TextBoxmonExp.Text.ToString.PadLeft(2, "0") + TextBoxdayExp.Text.ToString.PadLeft(2, "0")
            conn.Execute("update cl_kcbs set expiry_date = '" & dt & "' where acct = '" & acct_no & "' and period = (select ref_date from cl_refdate where sta = '00')")
            date_monthdiff_mnthday(dt, RefDate)
            txtArrear.Text = month + " Month/s " + day + " Day/s"
            ...
            If month = "00" Then
                ...
                Else
                    dm_flag = "d"
                    period = day.PadLeft(2, "0")
                End If
            Else
                dm_flag = "m"
                period = month.PadLeft(2, "0")
            End If

            If rs1.State = 1 Then
                rs1.Close()
            End If
            rs1.Open("SELECT NVL(name,'-') AS name FROM cl_objcri WHERE sta =(" & _
"SELECT MAX(sta) FROM cl_objcri WHERE d_m_flag3 = '" & dm_flag & "' AND cl3 <= '" & period & "')", conn, ADODB.CursorTypeEnum.adOpenKeyset)
            TextBoxObjCri.Text = rs1.Fields("NAME").Value.ToString
            ...
            ElseIf k = 30 Then
                txtBPAmt.Text = LabelOutstanding.Text - txtIntSus.Text
    ...
    Private Sub TextBoxAcno_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TextBoxAcno.TextChanged
        ...
        Me.VW_CL2TableAdapter.SelectCommand.CommandText = GetParam("fillcl2")
        Me.VW_CL2TableAdapter.Fill(DataSet3.VW_CL2, BranchCode, acct, "%" + TextBoxacname.Text + "%")
    End Sub
    ...
    Private Sub TextBoxacname_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TextBoxacname.TextChanged
        ...
        Me.VW_CL2TableAdapter.SelectCommand.CommandText = GetParam("fillcl2")
        Me.VW_CL2TableAdapter.Fill(DataSet3.VW_CL2, BranchCode, "%" + TextBoxAcno.Text.PadRight(13, "_"), acct)
    End Sub
    ...
    Private Sub ButtonSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ButtonSave.Click
        Try
           ...
           "',period_arear_mon='" & Trim(month) & "', period_arear_day = '" & Trim(day) & "'" & _
    ...
    Private Sub txtEligSec_KeyPress(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txtEligSec.KeyPress
        ...
        Else
            MessageBox.Show("Invalid amount. Only digits is allowed.")
            txtEligSec.Text = txtEligSec.Text
            ' Swallow this invalid key and beep
            e.Handled = True
        End If
 ...
 ByVal e As System.EventArgs) Handles TextBoxyear.LostFocus
    ...
    Private Sub GroupBoxDetails_Enter(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles GroupBoxDetails.Enter
